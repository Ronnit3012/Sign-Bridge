...
var NestedTreeControl = class extends BaseTreeControl {
  ...
  _getDescendants(descendants, dataNode) {
    ...
    } else if ((0, import_rxjs2.isObservable)(childrenNodes)) {
      childrenNodes.pipe((0, import_operators.take)(1), (0, import_operators.filter)(Boolean)).subscribe((children) => {
...
var CdkTree = class _CdkTree {
  ...
  _onDestroy = new import_rxjs2.Subject();
   ...
   */
  viewChange = new import_rxjs2.BehaviorSubject({
    start: 0,
   ...
   */
  _flattenedNodes = new import_rxjs2.BehaviorSubject([]);
  /** The automatically determined node type for the tree. */
  _nodeType = new import_rxjs2.BehaviorSubject(null);
  /** The mapping between data and the node that is rendered. */
  _nodes = new import_rxjs2.BehaviorSubject(/* @__PURE__ */ new Map());
  /**
   * Synchronous cache of nodes for the `TreeKeyManager`. This is separate
   * from `_flattenedNodes` so they can be independently updated at different
   * times.
   */
  _keyManagerNodes = new import_rxjs2.BehaviorSubject([]);
  ...
  _subscribeToDataChanges() {
    ...
    } else if ((0, import_rxjs2.isObservable)(this._dataSource)) {
      dataStream = this._dataSource;
    } else if (Array.isArray(this._dataSource)) {
      dataStream = (0, import_rxjs2.of)(this._dataSource);
    }
    if (!dataStream) {
  ...
  _initializeKeyManager() {
    const items = (0, import_rxjs2.combineLatest)([this._keyManagerNodes, this._nodes]).pipe((0, import_operators.map)(([keyManagerNodes, renderNodes]) => keyManagerNodes.reduce((items2, data) => {
  ...
  _getDirectChildren(dataNode) {
    ...
    const isExpanded = expansionModel.changed.pipe((0, import_operators.switchMap)((changes) => {
      if (changes.added.includes(key)) {
        return (0, import_rxjs2.of)(true);
      } else if (changes.removed.includes(key)) {
        return (0, import_rxjs2.of)(false);
      }
      return import_rxjs2.EMPTY;
    }), (0, import_operators.startWith)(this.isExpanded(dataNode)));
    if (levelAccessor) {
      return (0, import_rxjs2.combineLatest)([isExpanded, this._flattenedNodes]).pipe((0, import_operators.map)(([expanded, flattenedNodes]) => {
        if (!expanded) {
          return [];
        }
  ...
  _getAllChildrenRecursively(dataNode) {
    ...
    return coerceObservable(this.childrenAccessor(dataNode)).pipe((0, import_operators.take)(1), (0, import_operators.switchMap)((children) => {
      ...
      return (0, import_rxjs2.of)(...children).pipe((0, import_operators.concatMap)((child) => (0, import_rxjs2.concat)((0, import_rxjs2.of)([child]), this._getAllChildrenRecursively(child))));
    }));
  }
  ...
  _flattenNestedNodesWithExpansion(nodes, level = 0) {
    const childrenAccessor = this._getChildrenAccessor();
    if (!childrenAccessor) {
      return (0, import_rxjs2.of)([...nodes]);
    }
    return (0, import_rxjs2.of)(...nodes).pipe((0, import_operators.concatMap)((node) => {
      ...
      return (0, import_rxjs2.concat)((0, import_rxjs2.of)([node]), children.pipe((0, import_operators.take)(1), (0, import_operators.tap)((childNodes) => {
  ...
  _computeRenderingData(nodes, nodeType) {
    if (this.childrenAccessor && nodeType === "flat") {
      ...
      return (0, import_rxjs2.of)(nodes.filter((node) => levelAccessor(node) === 0)).pipe((0, import_operators.map)((rootNodes) => ({
  ...
  _forEachExpansionKey(callback) {
    ...
    if (observables.length > 0) {
      (0, import_rxjs2.combineLatest)(observables).pipe((0, import_operators.take)(1), (0, import_operators.takeUntil)(this._onDestroy)).subscribe((results) => {
...
var CdkTreeNode = class _CdkTreeNode {
  ...
  _dataChanges = new import_rxjs2.Subject();
...
//# sourceMappingURL=chunk-G5C7MO6Q.js.map
